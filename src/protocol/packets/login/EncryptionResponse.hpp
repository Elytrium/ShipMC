  ...
  class EncryptionResponse : public Packet {
   ...
   public:
    ...
    explicit EncryptionResponse(const PacketHolder& holder) {
      ...
      if (version >= &ProtocolVersion::MINECRAFT_1_19 && version < &ProtocolVersion::MINECRAFT_1_19_3 && !buffer->ReadBoolean()) {
        salt = buffer->ReadLong();
      }

      verifyToken = buffer->ReadByteArray(version >= &ProtocolVersion::MINECRAFT_1_19 ? 256 : 128);
    }
    ...
    void Write(const ProtocolVersion* version, ByteBuffer* buffer) const override {
      ...
      if (version >= &ProtocolVersion::MINECRAFT_1_19
          && version < &ProtocolVersion::MINECRAFT_1_19_3) {
        if (salt.has_value()) {
